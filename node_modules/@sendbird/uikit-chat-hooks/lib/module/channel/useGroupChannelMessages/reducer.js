    ...
    case 'update_next_messages':
      {
        ...
        if (action.value.clearPrev) {
          ...
          action.value.messages.filter(m => {
            return (m.isFileMessage() || m.isUserMessage()) && Boolean(draft[key][m.reqId]) && m.sendingStatus === 'succeeded' && isMyMessage(m, action.value.currentUserId);
...
export const useGroupChannelMessagesReducer = (userId, sortComparator) => {
  const [{
    loading,
    refreshing,
    messageMap,
